...
public class TestAStarPathfinder : MonoBehaviour
{
    [SerializeField] private int[,] tileInfoArray;
    private Vector2Int mapSize;

    private List<Vector2Int> openList;
    private List<Vector2Int> closedList;

    [SerializeField] int MaxPathSize = 10;
    [SerializeField] private Transform target;

    [SerializeField] private List<Vector2Int> lastPath;
    ...
    void OnEnable()
    {
        ...
        TestMapGenerator.instance.OnMapGenerated += Init;
    }
    ...
    public void Init()
    {
        TestMapGenerator mapGenerator = TestMapGenerator.instance;
        tileInfoArray = mapGenerator.TileInfoArray;
        mapSize = mapGenerator.MapSize;
        target = mapGenerator.player.transform;

        StartPathfinding();
    }

    public List<Vector2Int> StartPathfinding()
    {
        if (target == null)
            return null;

        Vector2Int startPos = ConvertWorldToMapPosition(transform.position);
        Vector2Int endPos = ConvertWorldToMapPosition(target.position);
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            Vector3 worldPos = ConvertMapToWorldPosition(tile);
